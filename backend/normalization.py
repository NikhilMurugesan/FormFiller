...
INTENT_PATTERNS = [
    ...
    ("desired_title", [r"desired.?title", r"target.?role", r"desired.?role", r"role.?applying", r"position.?applying", r"job.?applying"]),
    ...
    ("current_ctc", [r"\bcctc\b", r"current.?ctc", r"current.?salary", r"current.?compensation", r"present.?ctc", r"present.?salary"]),
    ("expected_ctc", [r"\bectc\b", r"expected.?ctc", r"expected.?salary", r"expected.?compensation", r"desired.?salary", r"desired.?ctc", r"salary.?expectation", r"compensation.?expectation", r"expected.?pay"]),
    ...
    ("cover_letter", [r"cover.?letter", r"motivation.?letter", r"letter.?of.?interest"]),
    ("message_to_recruiter", [r"message.?to.?recruiter", r"recruiter.?message", r"note.?to.?recruiter", r"short.?message"]),
    ("summary", [r"summary", r"about", r"why.?join", r"description"]),
...
PROFILE_ALIASES = {
    ...
    "desired_title": ["desired_title", "target_role", "primary_role"],
    "linkedin": ["linkedin"],
    "github": ["github"],
    "portfolio": ["portfolio"],
    "website": ["website"],
    ...
    "current_ctc": ["current_ctc", "current_salary", "current_compensation"],
    "expected_ctc": ["expected_ctc", "expected_salary", "expected_compensation", "desired_salary", "salary_expectation"],
    "salary_expectation": ["salary_expectation", "expected_ctc", "expected_salary", "expected_compensation", "desired_salary"],
    ...
    "message_to_recruiter": ["message_to_recruiter", "recruiter_message"],
    "summary": ["summary"],
    "skills": ["skills"],
    "gender": ["gender"],
    "ethnicity": ["ethnicity"],
...
def normalize_request(request: AnalyzeFieldsRequest) -> Dict[str, Any]:
    ...
    for field in request.detected_fields:
        ...
        normalized = {
            ...
            "label": clean_signal(field.label, 1000),
            "placeholder": clean_signal(field.placeholder, 600),
            "aria_label": clean_signal(field.aria_label, 600),
            "field_type": clean_text(field.field_type, 60),
            "input_tag": clean_text(field.input_tag, 40),
            "current_value": field.current_value,
            "candidate_options": options,
            "nearby_text": clean_signal(field.nearby_text, 800),
            "parent_section_text": clean_signal(field.parent_section_text, 1400),
            "section_heading": clean_signal(field.section_heading, 400),
