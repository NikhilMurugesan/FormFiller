  ...
  if (meta) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/document-status?session_id=${SESSION_ID}`, { signal: AbortSignal.timeout(3000) }
      );
      ...
      // Backend offline or timeout — warn quietly
...
document
  ...
  .addEventListener("change", async (e) => {
    ...
    try {
      ...
      setUploadStatus(
        `✅ ${data.chunk_count} chunks embedded & cached`,
        "info",
      );
    } catch (err) {
...
document.getElementById("autofillBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (response && response.status === "success") {
      const costText =
        response.cost !== undefined ? ` • $${response.cost.toFixed(6)}` : "";
      const timeText =
        response.latency !== undefined
          ? ` (${response.latency}s${costText})`
          : "";
...
document.getElementById("clearBtn").addEventListener("click", async () => {
  ...
  try {
    await chrome.storage.local.remove(STORAGE_KEY);
    await fetch(`${API_BASE}/clear-storage?session_id=${SESSION_ID}`, {
      method: "DELETE",
    });
